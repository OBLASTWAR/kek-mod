...
Controls.EspionageButtonBig:RegisterCallback(Mouse.eLClick, OnEspionageButton);
...
function DoUpdateEspionageButton()
	...
	if (iNumUnassignedSpies > 0) then
        ...
        Controls.UnassignedSpiesLabelBig:SetHide(false);
        Controls.UnassignedSpiesLabelBig:SetText(iNumUnassignedSpies);
	else
        Controls.UnassignedSpiesLabel:SetHide(true);
        Controls.UnassignedSpiesLabelBig:SetHide(true);
	end
	
    Controls.EspionageButton:SetToolTipString(strToolTip);
    Controls.EspionageButtonBig:SetToolTipString(strToolTip);
end
...
function CheckEspionageStarted()
    ...
    Controls.EspionageButton:SetHide(bLargeEspionage == 1 or not bEspionageStarted);
    Controls.EspionageButtonBig:SetHide(bLargeEspionage ~= 1 or not bEspionageStarted);
...
local function OnOptionsChanged()
    bLargeEspionage = EUI_options.GetValue('DB_bLargeEspionage') or 0;
    local bEspionageStarted = TestEspionageStarted();
    Controls.EspionageButton:SetHide(bLargeEspionage == 1 or not bEspionageStarted);
    Controls.EspionageButtonBig:SetHide(bLargeEspionage ~= 1 or not bEspionageStarted);
end
