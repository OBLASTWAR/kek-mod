...
#endif	//	_NDS
	// clang: 'long' is ambiguous between INT32 and INT64; resolve via cast
	void Write(const long & val) { Write((INT32)val); }
...
#endif	//	_NDS
	// clang: 'long' is ambiguous between INT32 and INT64; resolve via cast
	void Read(long & val) { INT32 tmp; Read(tmp); val = (long)tmp; }
	void Read(INT16 &);
	void Read(UINT16 &);
	void Read(WCHAR &);
	void Read(float &);
...
// rvalue-ref overload: allows temporaries like ArrayWrapper<T>(...) to bind
// (clang rejects binding temporaries to non-const lvalue refs; MSVC allowed it)
#ifdef __clang__
template<typename ValueType>
FDataStream & operator>>(FDataStream & loadFrom, ArrayWrapper<ValueType> && v)
{
