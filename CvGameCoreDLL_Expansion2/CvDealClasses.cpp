...
void CvDeal::AddDefensivePact(PlayerTypes eFrom, int iDuration)
{
	...
	if(IsPossibleToTradeItem(eFrom, GetOtherPlayer(eFrom), TRADE_ITEM_DEFENSIVE_PACT, iDuration))
	{
		...
		EraTypes eHighestEra = GET_PLAYER(eFrom).GetCurrentEra();
		...
		if(eHighestEra >= GC.getInfoTypeForString("ERA_RENAISSANCE", true /*bHideAssert*/))
		{
			item.m_iDuration = 10 /*iDuration*/;
		}
		else
		{
			item.m_iDuration = 20 /*2 * iDuration*/;
		}
#else
		item.m_iDuration = iDuration;
#endif
...
#ifdef DEF_PACT_COUNT
					GET_TEAM(eToTeam).incrementDevensivePactCount();
#endif
				}
				// Research Agreement
...
#ifdef RES_AGR_COUNT
					GET_TEAM(eToTeam).incrementResearchAgreementCount();
#endif
				}
				// Trade Agreement
...
#ifdef RES_AGR_COUNT
			GET_TEAM(eToTeam).decrementResearchAgreementCount();
#endif
		}
	}
	// Trade Agreement
