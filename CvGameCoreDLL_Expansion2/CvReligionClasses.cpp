...
void CvGameReligions::DoPlayerTurn(CvPlayer& kPlayer)
{
	...
	case FAITH_PURCHASE_SAVE_PROPHET:
		...
		if (eReligion <= RELIGION_PANTHEON && GetNumReligionsStillToFound() <= 0 && !(strcmp(kPlayer.getCivilizationTypeKey(), "CIVILIZATION_BYZANTIUM") == 0))
#else
...
CvGameReligions::FOUNDING_RESULT CvGameReligions::CanCreatePantheon(PlayerTypes ePlayer, bool bCheckFaithTotal)
{
	...
	for(it = m_CurrentReligions.begin(); it != m_CurrentReligions.end(); it++)
	{
		if(it->m_bEnhanced)
		{
			...
			{
#ifdef BYZANTIUM_CAN_ALWAYS_FOUND_RELIGION
				if (strcmp(kPlayer.getCivilizationTypeKey(), "CIVILIZATION_BYZANTIUM") == 0)
				{
					return FOUNDING_OK;
				}
				else
...
void CvGameReligions::FoundPantheon(PlayerTypes ePlayer, BeliefTypes eBelief)
{
	...
	{
		CvLuaArgsHandle args;
		args->Push(ePlayer);args->Push(GET_PLAYER(ePlayer).getCapitalCity()->GetID());
#ifdef AUI_RELIGION_FIX_FOUND_PANTHEON_NULL_POINTER_DEREFERENCE
	...
	if(pEntry && pBelief)
	{
		...
		for(int iNotifyLoop = 0; iNotifyLoop < MAX_MAJOR_CIVS; ++iNotifyLoop){
			...
			if(pNotifications){
				...
				{
					Localization::String localizedText = GetLocalizedText("TXT_KEY_NOTIFICATION_PANTHEON_FOUNDED_ACTIVE_PLAYER").GetCString();
...
bool CvGameReligions::CheckSpawnGreatProphet(CvPlayer& kPlayer)
{
	...
	if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && !kPlayer.isHuman())
	{
		return false;
	}
#endif
...
#ifdef BYZANTIUM_CAN_ALWAYS_FOUND_RELIGION
	else if(GetNumReligionsStillToFound() <= 0 && !(strcmp(kPlayer.getCivilizationTypeKey(), "CIVILIZATION_BYZANTIUM") == 0))
#else
...
CvCity *CvReligionAI::ChooseProphetConversionCity(bool bOnlyBetterThanEnhancingReligion) const
{
	...
	if (!bOnlyBetterThanEnhancingReligion)
	{
		for(int iPlayerLoop = 0; iPlayerLoop < MAX_CIV_PLAYERS; iPlayerLoop++)
		{
			...
			{
#ifdef NQM_AI_GIMP_NO_RELIGION_SPREAD
				if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && kLoopPlayer.isHuman())
					continue;
#endif
				int iCityLoop;
				for(pLoopCity = GET_PLAYER((PlayerTypes)iPlayerLoop).firstCity(&iCityLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER((PlayerTypes)iPlayerLoop).nextCity(&iCityLoop))
...
#ifdef BYZANTIUM_CAN_ALWAYS_FOUND_RELIGION
		if(pMyReligion == NULL && pReligions->GetNumReligionsStillToFound() <= 0 && !(strcmp(GET_PLAYER(m_pPlayer->GetID()).getCivilizationTypeKey(), "CIVILIZATION_BYZANTIUM") == 0))
#else
...
int CvReligionAI::ScoreCityForMissionary(CvCity* pCity, UnitHandle pUnit)
{
	...
	{
#ifdef NQM_AI_GIMP_NO_RELIGION_SPREAD
		if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && GET_PLAYER(pCity->getOwner()).isAlive())
			return 0;
#endif
	...
	for(int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
	{
		...
		if(kPlayer.isAlive())
		{
			...
			else
			{
				...
				{
#ifdef NQM_AI_GIMP_NO_RELIGION_SPREAD
					if (GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") && kPlayer.isAlive())
						continue;
#endif
